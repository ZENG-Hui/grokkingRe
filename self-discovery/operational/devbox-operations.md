...
- **⚠️ /opencode 节制使用** — 每次调用都会创建/延续 session，消耗资源
  - 能用 `/ls`、`/cat`、`/run` 解决的，**绝不用** `/opencode`
  - 测试/探查用一个 session 搞定，不要每次试都新开
  - 活跃 session_id 记到 `memory/hg-sessions.json`

### Session 复用策略（⚠️ 必须遵守）
**错误做法：** 每次调用 opencode 都新开 session → 堆积大量无用 session
**正确做法：** 按项目/模块划分 session，反复复用直到完成

示例：
```
teneral-thesis-chap01   ← 第一章所有编辑（多次调用，同一个 session）
...
规则：
- 一个项目/模块 = 一个 session，从头用到尾
- 编译 = 单独一个 session，所有编译复用
- 新开 session 前先想：有没有已有的 session 能复用？
- 每次创建新 session 立即记到 `memory/hg-sessions.json`
...
- **Session 命名规范：** 所有我开的 session 统一用 `teneral-` 前缀（如 `teneral-grokkingRe-fix-lr`），方便 Hui 区分哪些是我的、哪些是他自己的
